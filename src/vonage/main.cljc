(ns vonage.main
  "Kotodama WASM entrypoint for the Vonage clean-room actor (L4) — Clojure port.
  ...
  facts (`vonage.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "vonage")
(def tier "L4")
...
(def entity-specs
  [{:entity "Call"        :plural "calls"        :id-prefix "vonage_cal"
    :fields [:fromNumber :toNumber :status :direction :duration :price]
    :required [:fromNumber :toNumber]
    :coerce {:duration :int :price :float}  :refs {}}
   {:entity "Message"     :plural "messages"     :id-prefix "vonage_mes"
    :fields [:messageId :toNumber :fromNumber :status :messageType :channel]
    :required [:toNumber :fromNumber]
    :coerce {}  :refs {}}
   {:entity "Number"      :plural "numbers"      :id-prefix "vonage_num"
    :fields [:msisdn :country :type :cost]
    :required [:msisdn :country]
    :coerce {:cost :float}  :refs {}}
   {:entity "Verification" :plural "verifications" :id-prefix "vonage_ver"
    :fields [:requestId :number :status :codeLength]
    :required [:number :status]
    :coerce {:codeLength :int}  :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "vonage_app"
    :fields [:name :type :answerUrl]
    :required [:name :type]
    :coerce {}  :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"vonage.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vonage-compat" :tier tier :entities entities} 200])
...
(defn start! [] :vonage-compat/ready)
